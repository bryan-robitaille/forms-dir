...
const countNodes = (obj, root, c = 0) => {
  ...
  return u.direct_reports.reduce((s, a) => countNodes(a, root, s + 1), c);
};
...
export const computePositions = ({
  ...
  const computeLine = (parent, child, opt, buf) => {
    const getMiddle = (obj: NodeProfile) => {
      const middle =
        Math.floor(
          (Math.max.apply(
            null,
            obj.direct_reports.map((n) => n.col)
          ) +
            Math.min.apply(
              null,
              obj.direct_reports.map((n) => n.col)
    ...
    if (Math.abs(parent.y - child.y) === cardHeight + cardPadding) {
      // samerow
      // Move towards child
   ...
   *
   */
  const findNearestFreePosition = ({ row, col, size, a, b, l, r, anc, items }) => {
    const isXYOk = (y, x) => {
   ...
   */
  const addNode = (row, col, node, { a, b, l, r, override }) => {
    ...
    geometry.mid_col = geometry.right - Math.ceil((geometry.right - geometry.left) / 2);
    geometry.mid_row = geometry.bottom - Math.ceil((geometry.bottom - geometry.top) / 2);
   ...
   * @param {*} constraints
   */
  const addChildNodes = (row, col, node, { a, b, l, r, anc }) => {
    ...
    node.direct_reports.forEach((bNode) => {
      if (anc) {
        for (let n = 0; n < anc.length; n += 1) {
          if (anc[n].row === boxPosition.row + mod.y && anc[n].col === boxPosition.col + mod.x) {
            nextBox(mod);
            break;
          }
        }
      ...
      if (!bNode.placed) {
        addNode(boxPosition.row + mod.y, boxPosition.col + mod.x, bNode, { override: true });
        nextBox(mod);
      }
    });
    return geometry;
  ...
  const computePath = (criticalP) => {
    ...
    const moveNode = (node, newRow, newCol) => {
      if (node.col !== newCol || node.row !== newRow) {
        const tNode = node;
        rows[tNode.row].splice(rows[tNode.row].indexOf(tNode.col), 1);
    ...
    const occupiedBySibling = (node, row, col) => {
      const sNode = node.parent.direct_reports.filter((n) => n.col === col && n.row === row);
    ...
    const canNodeBeSwaped = (node, row, col) =>
      (occupiedBySibling(node, row, col) || !occupied(row, col)) && canMove(row, col);

    const swapChildren = (node, newRow, newCol) => {
      if (node.col !== newCol || node.row !== newRow) {
        const tNode = node;
        const sNode = node.parent.direct_reports.filter(
    ...
    if (criticalP.length > 0) {
      ...
      if (inter) {
        const newPos = Math.min(firstNode.direct_reports.length, Math.ceil(childrenMaxColumns / 2));
        firstNode.direct_reports.splice(firstNode.direct_reports.indexOf(inter), 1);
      ...
      path.slice(1).forEach((node) => {
        if (!node.parent.placed) {
          if (node.direct_reports.length > 0) {
            geometry = addChildNodes(geometry.top - 1, estimateStartColumn(geometry, node), node, {
              a: geometry.top + 1,
              anc,
            });
          }
          geometry = addNode(geometry.top - 1, geometry.mid_col, node, { a: geometry.top });
        } else if (node.direct_reports.length > 0) {
          const balanceL = rows.reduce((t, v) => t + v.filter((c) => c < 0).length, 0);
          const balanceR = rows.reduce((t, v) => t + v.filter((c) => c > 0).length, 0);
          ...
          const filterPotentials = (n) => {
            const box = findNearestFreePosition({ row: n.row + 1, col: n.col, size });
            return (
              (!(rows[n.row + 1] && rows[n.row + 1].includes(n.col)) ||
                !rows[n.row].includes(n.col - 1) ||
                !rows[n.row].includes(n.col + 1)) &&
              (!criticalP.includes(n) || n === node) &&
              box &&
              Math.abs((box.col < n.col ? box.col + size.x : box.col) - n.col) <= tries &&
          ...
          while (tries < 20) {
            ...
            if (potentialSwaps.length > 0) {
              ...
              if (node.col !== box.col && occupied(node.row + 1, node.col)) {
                moveNode(node, node.row, box.col < node.col ? box.col + size.x - 1 : box.col);
              }
              break;
            }
            tries += 1;
        ...
        } else if (node.col !== node.parent.col && node.parent.direct_reports.length > 1) {
          const nearest = node.parent.direct_reports
            .slice(0)
            .sort(
              (a, b) =>
    ...
    if (typeof node.x !== "undefined") {
      ...
      if (parent && typeof parent.x !== "undefined") {
        computeLine(
          ...
          { y: node.y, x: node.x },
          {
            on_path: withPath && criticalPath.includes(node) && criticalPath.includes(parent),
            child: node,
            parent,
          }
        );
  ...
  const xModifier = Math.abs(Math.min(...positionsMatrix.map((p) => p.node.x))) + leftGutter;
